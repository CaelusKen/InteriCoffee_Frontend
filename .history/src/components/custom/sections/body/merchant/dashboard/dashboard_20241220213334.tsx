  ...
  if(session && accountQuery.data?.data) {
    ...
    api.get<any>(`orders/merchant/${mappedAccount.merchantId}`).then((res) =>{
      if(res.status === 200) {
        ...
        setMerchantTemplates(mapBackendListToFrontend<Template>(templateQuery.data?.data.items, 'template').items.filter(item => item.merchantId.match(mappedAccount.merchantId)))
      }
    })
  }
