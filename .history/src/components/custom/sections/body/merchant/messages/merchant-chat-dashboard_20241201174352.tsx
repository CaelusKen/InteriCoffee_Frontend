...
export function MerchantFlow({ currentUser, selectedMerchant }: MerchantFlowProps) {
  ...
  const fetchChatSessions = async () => {
    ...
    try {
      ...
      // Filter sessions for the current merchant
      const merchantSessions = allSessions.filter(session => 
        session.advisorId === selectedMerchant.id
      )
      
      setChatSessions(merchantSessions)
    } catch (error) {
  ...
  const fetchConsultants = async () => {
    ...
    try {
      // Fetch all accounts and filter for consultants client-side
      const response = await api.get<Account[]>('accounts')
      const allAccounts = response.data.map(account => 
        mapBackendToFrontend<Account>(account, 'accounts')
      )
      
      // Filter for consultants associated with this merchant
      const merchantConsultants = allAccounts.filter(account => 
        account.role === 'CONSULTANT' && 
        account.merchantId === selectedMerchant.id
      )
      
      setConsultants(merchantConsultants)
    } catch (error) {
