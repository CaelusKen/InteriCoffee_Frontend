...
export default function MerchantOrderDetails({ id }: PackagingDetailsProps) {
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => {
        toast({
            description: `Packaging status updated to ${updateStatusMutation.data?.status}`,
            variant: "default",
            duration: 3000,
        })
        refetch()
  ...
  return (
    <Card className="max-w-4xl mx-auto my-8">
      ...
      <CardFooter className="bg-muted dark:bg-gray-700 flex justify-between items-center py-2">
        <Button
          variant="outline"
