...
const fetchCustomerChatSessions = async(accessToken: string): Promise<ApiResponse<PaginatedResponse<ChatSession>>> => {
  if(accessToken) return api.getPaginated<ChatSession>('chat-sessions', undefined, accessToken)
    else throw new Error(`Access token not available for ${accessToken}`)
}
...
export function MerchantSearch({ onSelect }: MerchantSearchProps) {
  ...
  const handleMerchantSelect = async (merchant: Merchant) => {
    ...
    console.log("Select merchant with id" + merchant.id)
