...
import { subscribeToChats, fetchChatById, createChat } from '@/lib/firebase-storage'
...
export default function MerchantChatPage() {
  ...
  const { data: merchantData, isLoading: isMerchantLoading, isError: isMerchantError } = useQuery({
    queryKey: ['merchant', account?.merchantId],
    queryFn: () => fetchMerchantById(account?.merchantId ?? '', accessToken ?? ''),
  ...
  useEffect(() => {
    if (account?.merchantId) {
      ...
      const unsubscribe = subscribeToChats(account?.merchantId, 'merchant',(updatedChats) => {
  ...
  const handleNewChat = async(customer: Account) => {
    toast({
      title: "Info",
      description: "Merchants cannot initiate new chats. Please wait for customers to contact you.",
    })
  }
  ...
  return (
    <div className="flex h-screen bg-background">
      ...
      />
      {showCustomerSearch ? (
        <CustomerSearch onSelectCustomer={(customer) => handleStartNewChat(customer)}/>
      ) : 
        selectedChat ? (
          <div className='max-w-full'>
            <MerchantChatMain 
              ...
              otherUser={selectedChat.participants.find(p => p.id !== merchant.id) as FirestoreChatParticipant}
            />
          </div>
      ) : (
