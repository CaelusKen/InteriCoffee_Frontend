...
import { BackendVoucher } from '@/types/backend/entities';

const entityHandlers = createEntityHandlers<BackendVoucher>('vouchers');
...
export async function GET(request: NextRequest) {
  ...
  if (data.data) {
    const mappedData = mapBackendListToFrontend<Voucher>(data.data, 'voucher');
...
export async function POST(request: NextRequest) {
  ...
  if (data.data) {
    const mappedData = mapBackendToFrontend<Voucher>(data.data, 'voucher');
    return NextResponse.json({ ...data, data: mappedData });
  }
